...
const webpageURL = 'https://www.peoplesearchsite.com/';
...
(async () => {
    try {
        ...
        await page.select('#st', state);
        await page.click("button[type=submit]");

        await page.waitForSelector('.table');

        const results = await page.evaluate(() => {
            let profileList = Array.from(document.querySelectorAll('tr')).slice(2, 10);
            let res = [];
            profileList.forEach((profile) => {
                let name = profile.querySelector('td:nth-child(1)')?.textContent?.trim() + ' ' + profile.querySelector('td:nth-child(2)')?.textContent?.trim() + ' ' + profile.querySelector('td:nth-child(3)')?.textContent?.trim();
                let address = profile.querySelector('td:nth-child(4)')?.textContent?.trim();
